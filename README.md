...
The commands below install released version `v0.2.8`. To install the moving
`main` branch instead, remove `#v0.2.8` from the Claude Code URL or remove
`--ref v0.2.8` from the Codex command. A floating install can change without
notice.

### Claude Code

```bash
claude plugin marketplace add antioch-robotics/antioch-agent-plugin.git#v0.2.8
...
```bash
codex plugin marketplace add antioch-robotics/antioch-agent-plugin.git --ref v0.2.8
