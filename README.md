...
The commands below pin plugin version `v0.2.25`. To install the moving `main`
branch instead, remove `#v0.2.25` from the Claude Code URL or remove
`--ref v0.2.25` from the Codex command. A floating install can change without
notice.

### Claude Code

```bash
claude plugin marketplace add antioch-robotics/antioch-agent-plugin#v0.2.25
...
```bash
codex plugin marketplace add antioch-robotics/antioch-agent-plugin --ref v0.2.25
