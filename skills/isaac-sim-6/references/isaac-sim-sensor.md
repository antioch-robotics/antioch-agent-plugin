...
| Mobile-robot SDG | `isaacsim.replicator.experimental.mobility_gen` |
...
| Domain randomization helpers | `isaacsim.replicator.experimental.domain_randomization` (the non-experimental `isaacsim.replicator.domain_randomization` is deprecated) |
...
|---|---|---|
| [`scripts/attach_lidar_imu.py`](https://github.com/isaac-sim/IsaacSim/blob/7c206f75bdadd9e05fc457f19863ca4c3f0cb693/skills/isaac-sim-sensor/scripts/attach_lidar_imu.py) | Attach an RTX Ouster LiDAR and a physics IMU to a robot chassis | see script --help |
| [`scripts/create_camera_sensor.py`](https://github.com/isaac-sim/IsaacSim/blob/7c206f75bdadd9e05fc457f19863ca4c3f0cb693/skills/isaac-sim-sensor/scripts/create_camera_sensor.py) | Camera sensor creation and annotator attachment for Isaac Sim Replicator | see script --help |
| [`scripts/lidar_gmo_writer.py`](https://github.com/isaac-sim/IsaacSim/blob/7c206f75bdadd9e05fc457f19863ca4c3f0cb693/skills/isaac-sim-sensor/scripts/lidar_gmo_writer.py) | RTX Lidar GMO (GenericModelOutput) writer pattern for Isaac Sim | see script --help |

## Related skills

- [isaac-camera](isaac-camera.md): deep dive on `RtxCamera` / `CameraSensor`, calibration, distortion.
- [isaac-sim-rendering](isaac-sim-rendering.md): headless capture pipeline, RT2, ACES tonemap.
- [physics-simulation](physics-simulation.md): scene config + physics sensors (`Contact`, `IMU`, etc.).
- [data-collection-sim](data-collection-sim.md): static-scene SDG writer pipelines.
- [mobility-gen](mobility-gen.md): mobile-robot trajectory-driven SDG.
...
For depth / stereo cameras (Intel RealSense D415/D435/D455/L515, Stereolabs ZED 2/2i/Mini/X), use the camera-style `RtxCamera` wrapper plus `SingleViewDepthCameraSensor`; see [isaac-camera](isaac-camera.md).
...
`RtxCamera` + `CameraSensor` is the camera-authoring/capture path (there is no supported non-RTX authoring path). See [isaac-camera](isaac-camera.md) for the full surface.
...
See [`scripts/create_camera_sensor.py`](https://github.com/isaac-sim/IsaacSim/blob/7c206f75bdadd9e05fc457f19863ca4c3f0cb693/skills/isaac-sim-sensor/scripts/create_camera_sensor.py).
...
See [`scripts/lidar_gmo_writer.py`](https://github.com/isaac-sim/IsaacSim/blob/7c206f75bdadd9e05fc457f19863ca4c3f0cb693/skills/isaac-sim-sensor/scripts/lidar_gmo_writer.py).
...
Attach a `Writer` to consume GMO — see Section 2. Motion BVH must be enabled (`SimulationConfig(extra_args=["--/renderer/raytracingMotion/enabled=true"])` before startup).
...
with a bounded deadline. `get_data()` can retain a previous frame on an invalid
read. Filter sizes are sample counts; IMU has no `tick_rate` or annotator list.
...
The experimental `Contact.create` / `ContactSensor` path needs an enabled rigid-body ancestor and collision geometry; author the sensor at a child path (as above), not at the collider/body path itself. The deprecated classic `isaacsim.sensors.physics.ContactSensor` additionally requires the immediate parent to carry `UsdPhysics.CollisionAPI`, and its owning extension `isaacsim.sensors.physics` (shipped under `extsDeprecated`) must be enabled via `SimulationConfig.extensions` — it is not enabled by default.

`EffortSensor` targets one joint; `JointStateSensor` targets the actual
articulation root. Both are runtime-only. After play and physics steps,
`get_data()` returns a dictionary: check `is_valid` and `time` before using it.
Use `get_sensor_reading()` for the structured reading object.
...
with rep.new_layer():
    ...
    with rep.trigger.on_frame(num_frames=100):
        with lights:
            ...
            rep.modify.attribute("color", rep.distribution.uniform((0.8,) * 3, (1.0,) * 3))
...
writer.initialize(output_dir="/output/synthetic_dataset", semantic_types=["class"], write_binary_pointcloud=False, mapping={"box": "Car", "pallet": "Misc"})
...
|---|---|
| Static-scene SDG with randomization | [data-collection-sim](data-collection-sim.md) skill + `isaacsim.replicator.examples` (`sdg_getting_started_0[1-5].py`, `sdg_workflow_0[12].py`) |
...
- `aux_output_level` levels are modality-specific: lidar supports `NONE`/`BASIC`/`EXTRA`/`FULL`; radar/acoustic only `NONE`/`BASIC`. `RtxCamera` does not accept this keyword.
