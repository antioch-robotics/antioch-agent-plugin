...
An Antioch machine is an ephemeral GPU VM assigned to one user. The
...
Machine list and status JSON use the documented user schema: identity, direct
...
selection. All `*_at` values are Unix microseconds. See `cli.md` for the
...
and never assign one. When no warm machine is available, the CLI itself waits
...
results, telemetry, or artifacts need to be saved after the process exits.
...
Scenario artifacts and asset payloads accept up to 16 GiB per object. Antioch
does not expose multipart uploads or byte-range resume, so bound the failure
domain by splitting a larger dataset into archives such as
`shard-0000.tar.zst` through `shard-0015.tar.zst`.
...
If a shard fails, retry only that shard. Compare the SHA-256 in Antioch's JSON
transfer result with the dataset manifest before marking it complete. Keep the
manifest itself as a small scenario artifact or versioned asset so the list of
completed shards is durable.
